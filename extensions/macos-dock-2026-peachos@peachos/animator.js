...
export let Animator = class {
  ...
  _precreateResources(dock) {
    ...
    let separatorCount = (dock._separators || []).length;
    while (this._separatorRenderers.length < separatorCount) {
      let sep = new St.Widget({
        name: 'd2daSeparator',
        reactive: false,
      });
      dock.renderArea.add_child(sep);
      this._separatorRenderers.push(sep);
    }
  ...
  animate(dt) {
    ...
    // separators: the dash widget is a layout ghost (icons drawn in
    // renderArea). Paint the line there so it stays visible at rest,
    // not only while hover animation is running.
    let separatorRgba = dock.extension._style
      ? dock.extension._style.rgba(dock.extension.separator_color)
      : '255,255,255,0.4';
    let thickness = dock.extension.separator_thickness || 0;
    dock._separators.forEach((actor, i) => {
      actor.opacity = 0;
      let overlay = this._separatorRenderers[i];
      if (!overlay) {
        return;
      }
      if (thickness <= 0) {
      ...
      if (!vertical) {
        actor.width = size.width + 8;
        actor.height = 1;
      } else {
        actor.height = size.height + 8;
        actor.width = 1;
      }
      ...
      let prevR = prev && prev._renderer;
      let nextR = next && next._renderer;
      if (prevR && nextR && prevR.visible && nextR.visible) {
        if (!vertical) {
          let prevRight = prevR.x + prevR.width * (prevR.scaleX || 1);
          let nextLeft = nextR.x;
          overlay.x = (prevRight + nextLeft) / 2 - overlay.width / 2;
          overlay.y =
            prevR.y +
            (prevR.height * (prevR.scaleY || 1) - overlay.height) / 2;
        } else {
          let prevBottom = prevR.y + prevR.height * (prevR.scaleY || 1);
          let nextTop = nextR.y;
          overlay.y = (prevBottom + nextTop) / 2 - overlay.height / 2;
          overlay.x =
            prevR.x + (prevR.width * (prevR.scaleX || 1) - overlay.width) / 2;
        }
