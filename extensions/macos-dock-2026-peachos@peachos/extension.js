...
import { buildDockBackgroundStyle, modeForColorScheme } from './liquidGlass.js';
import { SIZE_SLIDER_DEFAULT } from './iconMetrics.js';
...
export default class Dash2DockLiteExt extends Extension {
  ...
  enable() {
    ...
    this.listeners = [];
    this.scale = 1.0;
    this.icon_size = SIZE_SLIDER_DEFAULT;
    ...
    // Size slider now scales icons and dock together. Kill Shrink Icons so
    // it cannot fight the slider, and remap the old default (0 = 32px).
    if (this.shrink_icons) {
      this._settingsKeys.setValue('shrink-icons', false);
      if ((this.icon_size || 0) <= 0) {
        this._settingsKeys.setValue('icon-size', SIZE_SLIDER_DEFAULT);
      }
    }
    ...
    console.log('macos-dock-2026-peachos enabled');
  ...
  disable() {
    ...
    console.log('macos-dock-2026-peachos disabled');
  }
  ...
  _enableSettings() {
    ...
    // Liquid Glass mode isn't a user-facing choice (see preferences/keys.js --
    // 'liquid-glass-mode' has no widget in the Settings app's Docks tab), it's
    // driven entirely by the system's light/dark appearance setting so the
    // dock always matches everything else. Sync once here for the value
    // already in effect at enable() time (login, or the extension reloading)
    // -- the changed::color-scheme listener above only fires on *later*
    // changes, so without this the dock could start in the wrong mode until
    // the user actually flips their appearance setting.
    this._syncLiquidGlassMode();
  }

  // Keeps 'liquid-glass-mode' (see liquidGlass.js) in lockstep with
  // org.gnome.desktop.interface's color-scheme, the same key/values
  // macOS-TopBar-Gnome's own AppearanceController watches for the same
  // purpose. Routed through _settingsKeys.setValue() rather than a direct
  // settings.set_int() so it goes through the exact same path a user's own
  // change would (fires the 'changed::liquid-glass-mode' handler above,
  // updates this.liquid_glass_mode, repaints via _updateStyle()).
  _syncLiquidGlassMode() {
    if (!this._interfaceSettings || !this._settingsKeys) return;
    const mode = modeForColorScheme(this._interfaceSettings.get_string('color-scheme'));
    if (this._settingsKeys.getValue('liquid-glass-mode') !== mode) {
      this._settingsKeys.setValue('liquid-glass-mode', mode);
    }
  }
  ...
  _updateStyle(disable) {
    ...
    // dash
    {
      ...
      styles.push(`#d2daBackground { ${dashStyle} }`);
    }
    ...
    if (this.separator_thickness) {
      ...
      styles.push(
        `.dash-separator { background-color: rgba(${rgba}) !important; }`
      );
    }
  ...
  _updateBorderStyle() {
    ...
    let border_style = '';
    // Liquid glass already paints its own rim; stacking the user border
    // on top would flatten the highlight.
    if (!this.liquid_glass && this.border_thickness /* && !this._disable_borders */) {
