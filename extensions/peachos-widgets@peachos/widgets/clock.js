...
export class AnalogClock {
    constructor(parent, size) {
        this._fg = size.fg || '255,255,255';
        this._glass = this._fg.startsWith('255') && !(size.mode === 'dark');
        this._root = new St.Widget({
            layout_manager: new Clutter.BinLayout(),
            width: size.w, height: size.h,
        });
    ...
    _layoutNumbers(w, h) {
        ...
        for (let i = 1; i <= 12; i++) {
            ...
            const l = new St.Label({
                text: `${i}`,
                style: fontStyle(FONT.rounded, fs, 0.9, this._fg),
            });
    ...
    _draw(area) {
        ...
        try {
            ...
            // Plate: translucent in glass, opaque disc in solid modes
            if (this._glass)
                cr.setSourceRGBA(...this._rgb(0.20));
            else
                cr.setSourceRGBA(...this._rgb(0.14));
            ...
            for (let i = 0; i < 60; i++) {
                cr.setSourceRGBA(...this._rgb(i % 5 === 0 ? 0.75 : 0.30));
