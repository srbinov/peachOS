    ...
    def _build_ui(self):
        self.append(make_hero_header(
            ...
            'Accessibility', 'Personalize peachOS in ways that work best for you, with accessibility '
                              'features for vision and hearing.',
        ))
        ...
        self.append(GroupCard([
            IconPlaceholderRow(
                'VoiceOver', 'audio-speakers-symbolic', 'accent-black', switch=self._voiceover_switch,
                icon_file=os.path.join(ICON_DIR, 'accessibility_voiceover.svg'),
            ),
            IconPlaceholderRow(
                'Zoom', 'zoom-in-symbolic', 'accent-black', on_click=self._on_open_zoom,
                icon_file=os.path.join(ICON_DIR, 'accessibility_zoom.svg'),
            ),
            IconPlaceholderRow(
                'Display', 'video-display-symbolic', 'accent-blue', on_click=self._on_open_display,
                icon_file=os.path.join(ICON_DIR, 'accessibility_display.svg'),
            ),
            IconPlaceholderRow(
                'Motion', 'object-rotate-right-symbolic', 'accent-green', switch=self._motion_switch,
                icon_file=os.path.join(ICON_DIR, 'accessibility_motion.svg'),
            ),
        ]))
