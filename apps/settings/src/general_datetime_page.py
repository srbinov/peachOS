...
class DropdownRow(Gtk.Box):
    def __init__(self, title: str, options: list, enable_search: bool = False):
        ...
        if enable_search:
            # Pins the popup's list to a fixed height (see main.py's
            # STYLE_CSS) so it doesn't resize -- and visibly jump/bounce
            # to a different screen position -- as search narrows the
            # row count while typing.
            self.dropdown.add_css_class('searchable-dropdown')
