...
class DropdownRow(Gtk.Box):
    """Time Zone has ~450 options -- Gtk.DropDown's own enable-search is a
    real filter (hides non-matching rows), which the user explicitly
    didn't want: all options should stay visible, with typing just
    jumping the selection to the best match. See widgets.TypeaheadDropdown."""
    ...
    def set_options(self, options: list, selected: str = None):
        self.dropdown.set_options(options, selected=selected)
...
class GeneralDateTimePage(Gtk.Box):
    ...
    def _build_ui(self):
        ...
        self._tz_row = DropdownRow('Time Zone', [])
        self._tz_row.connect_changed(self._on_timezone_changed)
    ...
    def _on_timezone_changed(self, tz):
        if self._syncing or not self._bus or not tz:
            return
        try:
            self._bus.call_sync(
